...
} from '../../../../features/edit-lead/model/lead-edit-form.helpers';
import {
   fetchLeadById,
   updateLead,
} from '../../../../entities/lead/api/leads.api';
import { deleteLeadCargo } from '../../../../entities/lead/api/leads.api';
...
export function useLeadDetailsMutations({
   ...
   async function reloadCurrentLead() {
      ...
      const response = await fetchLeadById(currentLead.id);
   ...
   async function handleDeleteCargo(cargoIndex) {
      ...
      try {
         ...
         await deleteLeadCargo(currentLead.id, cargoIndex);
         await reloadCurrentLead();
